#!/usr/bin/env python3
"""
Test script for first 5 medium complex consent questions with automatic JSON output
Tests Q1-Q5 from medium_complex_questions.md and saves results for analysis
"""
import asyncio
import sys
import json
...
# First 5 medium complex questions from medium_complex_questions.md - Jurisdiction-anchored
MEDIUM_QUESTIONS = [
    {
        "id": "Q1",
        "question": "According to Estonia's data protection law, what specific conditions make consent valid? Include requirements from Costa Rica and Denmark if similar.",
        "expected_keywords": ["freely given", "specific", "informed", "unambiguous", "Estonia", "Costa Rica", "Denmark", "coercion", "pressure", "clear action", "revocable", "withdraw"]
    },
    {
        "id": "Q2",
        "question": "Based on Costa Rica's Article 8 and similar provisions in Denmark, what documentation must data controllers maintain when managing consent?",
        "expected_keywords": ["Costa Rica", "Article 8", "Denmark", "demonstrate", "record", "consent", "maintain", "version", "scope", "time", "revocation", "mechanisms"]
    },
    {
        "id": "Q3",
        "question": "According to Denmark's consent requirements and Costa Rica's Article 6, what are three specific errors organizations make about consent?",
        "expected_keywords": ["Denmark", "Costa Rica", "Article 6", "silence", "consent", "bundling", "terms", "conditions", "separate", "purpose", "granular", "affirmative action"]
    },
    {
        "id": "Q4",
        "question": "How do Estonia, Costa Rica (Article 8), and Denmark specifically handle consent withdrawal, including any time limits mentioned?",
        "expected_keywords": ["Estonia", "Costa Rica", "Article 8", "Denmark", "revoke", "withdraw", "5 days", "delete", "restrict", "processing", "easy", "time"]
    },
    {
        "id": "Q5",
        "question": "What specific practices do Costa Rica Article 10, Denmark, and Estonia require for demonstrating valid consent collection and auditing?",
        "expected_keywords": ["Costa Rica", "Article 10", "Denmark", "Estonia", "log", "metadata", "date", "method", "scope", "purpose", "traceability", "audit trails"]
    }
]
...
class MediumQuestionTester:
    ...
    def __init__(self):
        ...
        self.test_metadata = {
            ...
            "test_script": "test_medium_questions.py"
        }
    ...
    async def run_all_tests(self):
        ...
        console.print("\n[bold cyan]Testing First 5 Medium Complex Questions Through Pipeline[/bold cyan]")
        console.print("=" * 80)
        ...
        console.print("\n[bold]Testing In-Scope Medium Complex Consent Questions:[/bold]")
        ...
        # Save to JSON file
        output_file = Path(__file__).parent / f"medium_questions_test_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def show_summary(self, out_of_scope_results: List[Dict]):
        ...
        table = Table(title="Medium Complex Consent Questions Results")
