...
These are not covered in the current document set. Please refine your query to focus on consent-related matters, or consult the full regulatory texts directly.""",
    ...
    def should_use_fallback(self, search_results: List, query_analysis: Dict) -> bool:
        ...
        print(f"DEBUG FALLBACK: Average score: {avg_score}, Threshold: 0.005")
        if avg_score < 0.005:
            print(f"DEBUG FALLBACK: TRIGGERING fallback due to low score {avg_score}")
            logger.info(f"Using fallback: Low average relevance score {avg_score}")
            return True
        else:
            print(f"DEBUG FALLBACK: NOT triggering fallback, score {avg_score} >= 0.005")
    ...
    def enhance_response_with_fallback(
        ...
        Returns:
            Enhanced response
        """
        print(f"DEBUG ENHANCE: Original response length: {len(original_response)} chars")
        print(f"DEBUG ENHANCE: Fallback content length: {len(fallback.content)} chars")
